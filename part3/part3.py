...
# terminals
pset.addEphemeralConstant("1rand100", lambda: random.random() * 100, float)
pset.addEphemeralConstant("2rand100", lambda: random.random() * 100, float)
...
def main(datafile):
    ...
    pop, log = algorithms.eaSimple(pop, toolbox, 0.8, 0.05, 100, stats=mstats,
                                   halloffame=hof, verbose=True)
    print("\n\ntraining: " + str(hof.keys[0]) + "/" + str(len(training)) + "-->" + str(hof.items[0]))
    print("\n" "test: " + str(evalClassif(hof.items[0], test)) + "/" + str(len(test)))
